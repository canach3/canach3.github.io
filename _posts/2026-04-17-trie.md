...
tags: [data structure, 자료구조, trie, 트라이]
description: "트라이(Trie)에 대한 개념을 알아보고 코드로 구현해보자"
image:
...
따라서 전체 단어의 개수를 **$N$**, 단어의 최대길이를 **$L$** 이라고 할 때, 시간복잡도는 $O(N×L)$ 이 된다. 위 예시에서 단어의 최대길이가 **10**이라면   
...
2. **한 글자씩 자식 노드로 이동 :** 찾고자 하는 단어의 첫 문자부터 순서대로 자식 노드를 따라 내려간다.  **`a` → `l` → `g` → `o`** → …. 순서로 이동한다.  
**만약 중간에 다음 문자로 이어지는 노드가 없다면, 길이 끊긴 것이므로 탐색을 즉시 중단**하고 해당 단어가 없다고 판단한다. 
...
3. **단어의 끝 표시 확인 :** 단어의 마지막 글자인 **`m`**까지 무사히 도달했다면, 마지막으로 해당 노드에 **'단어의 끝(`isEndOfWord`)' 표시가 되어 있는지 확인한다.** 표시가 되어 있다면 검색 성공이다. 
    ...
    (예: **`algo`** 라는 단어를 검색했을 때, 마지막 글자인 **`o`** 에는 **`isEndOfWord`** 가 표시되어있지 않으므로, 검색 실패처리가 된다.)  
    (**`algo`**는 **`algorithm`** 의 접두사로서만 존재)
