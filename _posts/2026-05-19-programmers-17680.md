...
1. **캐시에 특정 도시 존재여부 확인**
    ...
    : 리스트(캐시) 내에 특정 도시 이름이 있는지 찾기
    
    → 위해 처음부터 끝까지 순회해야 하므로 $O(N)$의 시간이 소요된다.
...
2. **초기화 및 더미 노드 세팅**
    ...
    - `head`의 바로 뒤(`head.next`)는 가장 오래된 데이터(LRU)가 위치한다.
    - `tail`의 바로 앞(`tail.prev`)은 가장 최신 데이터(MRU)가 위치한다.
3. **Cache Hit 상황**
