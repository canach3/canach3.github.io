...
우리는 직접 구현한 **이중 연결 리스트**를 통해 LRU 알고리즘을 구현할 것이다. **가장 최근에 사용된 데이터는 리스트의 맨 뒤(tail)**에, **가장 오랫동안 사용되지 않은 데이터는 리스트의 맨 앞(head)에 위치시켜 관리한다.**
...
- `map.get(key) != null` -> 캐시에 해당 데이터 존재 O (해당 이름의 도시가 존재함)
- `map.get(key) == null` -> 캐시에 해당 데이터 존재 X (해당 이름의 도시가 존재하지 않음)
